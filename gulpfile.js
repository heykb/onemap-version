...
var gulpVersion = require('./onemap-verison.js');
...
var config = {
  ...
  // 是否zip包输出
  outZipEnable: true,
  // 是否禁止输出项目所有文件，true: 只输出srcPattern匹配的文件（大大节约时间）
...
gulp.task('transform', function () {
  if(config.outAllRepDisable){
    config.srcPattern = config.srcPattern.map(item=>{
      if(item.startsWith('!'))return '!'+config.srcRoot+'/'+item.substring(1);
      else return config.srcRoot+'/'+item;
    })
    var stream = gulp.src(config.srcPattern)
      .pipe(gulpif(['**/seajsInit.js', '**/initPortal.js'], gulpVersion.seajsVersion(options)))          
    ...
    var stream = gulp.src(config.srcRoot + '/**/*')
          .pipe(gulpif(['**/seajsInit.js', '**/initPortal.js'], gulpVersion.seajsVersion(options)))
          .pipe(gulpif(config.srcPattern,gulpVersion.gulpVersion(options),log()))
    if (config.outZipEnable) {
      stream = stream.pipe(zip('archive.zip'));
    }
    return stream.pipe(gulp.dest(config.outRoot))
...
gulp.task('cleanDist', function () {
  return gulp.src(config.outRoot,
    {
      read: false,
      allowEmpty: true
    })
